...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, microBolusAllowed, reservoir_data) {
    ...
    rT.predBGs = {};
    IOBpredBGs = _.map(IOBpredBGs, p => round(clamp(p, 39, 401)));
    ...
    ZTpredBGs = _.map(ZTpredBGs, p => round(clamp(p, 39, 401)));
    ...
    if (meal_data.mealCOB > 0) {
        aCOBpredBGs = _.map(aCOBpredBGs, p => round(clamp(p, 39, 401)));
    ...
    if (meal_data.mealCOB > 0 && ( ci > 0 || remainingCIpeak > 0 )) {
        COBpredBGs = _.map(COBpredBGs, p => round(clamp(p, 39, 401)));
    ...
    if (ci > 0 || remainingCIpeak > 0) {
        if (enableUAM) {
            UAMpredBGs = _.map(UAMpredBGs, p => round(clamp(p, 39, 401)));
