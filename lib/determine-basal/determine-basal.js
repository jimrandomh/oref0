...
const xdripErrorCodeBG = 38; // 38 is an xDrip error state that usually indicates sensor failure
const maxAllowedDataAge = 12; // Abort if CGM data is older than this
...
const maxRemainingCarbsCap = 90;
const microbolusMinSpacingMins = 3; // Minimum number of minutes between microboluses
...
function autosens_adjust_targets(autosens_data, profile, targetRange)
{
    var [min_bg, max_bg, target_bg] = targetRange;
    ...
    if (profile.sensitivity_raises_target && autosens_data.ratio < 1 || profile.resistance_lowers_target && autosens_data.ratio > 1)
    {
        ...
        return [min_bg, max_bg, new_target_bg];
    }
    else
    {
        return targetRange;
...
function noisy_cgm_adjust_targets(profile, targetRange)
{
    var [min_bg, max_bg, target_bg] = targetRange;
    ...
    return [adjustedMinBG, adjustedMaxBG, adjustedTargetBG];
}

function apply_adv_target_adjustment(profile, bg, naive_eventualBG, eventualBG, targetRange)
{
    var [min_bg, max_bg, target_bg] = targetRange;
    ...
    return [min_bg, max_bg, target_bg];
}

function smb_get_max_bolus(profile, iob_data, meal_data)
{
    ...
    if (typeof profile.maxSMBBasalMinutes == 'undefined' ) {
        ...
        return round( profile.current_basal * 30 / 60 ,1);
    // if IOB covers more than COB, limit maxBolus to 30m of basal
        ...
        if (profile.maxUAMSMBBasalMinutes) {
            ...
            return round( profile.current_basal * profile.maxUAMSMBBasalMinutes / 60 ,1);
        } else {
        ...
        return round(profile.current_basal * profile.maxSMBBasalMinutes / 60, 1);
    }
}
...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, microBolusAllowed, reservoir_data) {
    ...
    if (bg <= 10 || bg == xdripErrorCodeBG || noise >= 3) {  //Dexcom is in ??? mode or calibrating, or xDrip reports high noise
        abortReason = "CGM is calibrating, in ??? state, or noise is high";
    }
    if (minAgo > maxAllowedDataAge || minAgo < -5) { // Dexcom data is too old, or way in the future
    ...
    if (profile.temptargetSet) {
        ...
        [min_bg, max_bg, target_bg] = autosens_adjust_targets(autosens_data, profile, [min_bg, max_bg, target_bg]);
    }
    ...
    if (glucose_status.noise >= 2) {
        [min_bg, max_bg, target_bg] = noisy_cgm_adjust_targets(profile, [min_bg, max_bg, target_bg]);
    // adjust target BG range if configured to bring down high BG faster
        ...
        [min_bg, max_bg, target_bg] = apply_adv_target_adjustment(profile, bg, naive_eventualBG, eventualBG, [min_bg, max_bg, target_bg]);
    }
    ...
    var remainingCarbsCap = maxRemainingCarbsCap; // default to 90
    var remainingCarbsFraction = 1;
    if (profile.remainingCarbsCap) { remainingCarbsCap = Math.min(maxRemainingCarbsCap,profile.remainingCarbsCap); }
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            var maxBolus = smb_get_max_bolus(profile, iob_data, meal_data);
            
            // bolus 1/2 the insulinReq, up to maxBolus, rounding down to nearest 0.1U
            var microBolus = Math.floor(Math.min(insulinReq/2,maxBolus)*10)/10;
            // calculate a long enough zero temp to eventually correct back up to target
            ...
            var nextBolusMins = round(microbolusMinSpacingMins-lastBolusAge,0);
            var nextBolusSeconds = round((microbolusMinSpacingMins - lastBolusAge) * 60, 0) % 60;
            ...
            if (lastBolusAge > microbolusMinSpacingMins) {
                if (microBolus > 0) {
                    rT.units = microBolus;
