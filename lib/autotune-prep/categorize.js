...
var calcTempTreatments = require('../iob/history');
...
function categorizeBGDatums(opts) {
    var treatments = _.orderBy(opts.treatments,
        t => new Date(tz(t.timestamp)).getTime(), ['desc']);
    ...
    glucoseData = _.orderBy(glucoseData, g => new Date(tz(g.date)).getTime(), ['desc']);
    ...
    var treatments = calcTempTreatments(IOBInputs);
    ...
    if (opts.categorize_uam_as_basal) {
        ...
        basalGlucoseData = _.sortBy(basalGlucoseData, g => g.deviation);
