...
describe('determine-basal', function ( ) {
   ...
   //function determine_basal(glucose_status, currenttemp, iob_data, profile)
    ...
    it('should let low-temp run when bg < 30 (Dexcom is in ???)', function () {
        var currenttemp = {"duration":30,"rate":0,"temp":"absolute"};
        var output = determine_basal({glucose:10},currenttemp, iob_data, profile, undefined, meal_data);
        //console.log(output);
        (typeof output.rate).should.equal('undefined');
    ...
    //it('iob_data should contain activity, iob, bolussnooze', function () {
      //var result = determine_basal({glucose:100}, undefined,{"activity":0}, {"current_basal":0.0, "max_bg":100,"min_bg":110}, undefined, meal_data);
...
/*
    it('should return error eventualBG if something went wrong', function () {
      var result = determine_basal({glucose:100}, undefined,{"activity":0, "iob":0,"bolussnooze":0}, {"current_basal":0.0, "sens":NaN}, undefined, meal_data);
...
*/
    ...
    */
    ...
    it('should temp to zero with double sensitivity adjustment', function () {
        ...
        var output = determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data);
        //console.log(output);
        output.rate.should.equal(0);
        output.duration.should.equal(30);
    });
	...
	var currenttemp = {"duration":0,"rate":0,"temp":"absolute"};
        var output = determine_basal(glucose_status, currenttemp, iob_data, profile2, undefined, meal_data);
